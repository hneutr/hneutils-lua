...
describe("expanduser", function()
    ...
    it("doesn't expand anywhere else", function()
        assert.are.equal("/a/~/b", Path.expanduser("/a/~/b"))
    end)
end)
...
describe("is_relative_to", function()
    it("root", function()
        assert(Path.is_relative_to("/a/b", "/"))
    end)

    it("parent", function()
        assert(Path.is_relative_to("/a/b", "/a"))
    end)

    it("parent's parent", function()
        assert(Path.is_relative_to("/a/b/c", "/a"))
    end)

    it("non-parent", function()
        assert.is_false(Path.is_relative_to("/a/b/c", "/d"))
    end)
end)

describe("relative_to", function()
    it("root", function()
        assert.are.same("a", Path.relative_to("/a", "/"))
    end)

    it("parent", function()
        assert.are.same("b", Path.relative_to("/a/b", "/a"))
    end)

    it("parent's parent", function()
        assert.are.same("c", Path.relative_to("a/b/c", "a/b"))
    end)

    it("non-relative fails", function()
        assert.has_error(function() Path.relative_to("/a", "b") end, "/a is not relative to b")
    end)
end)
...
describe("rename", function()
    it("file", function()
        Path.mkdir(test_dir_path)
        Path.write(test_dir_file_path, "123")
        Path.rename(test_dir_file_path, test_file_path)
        assert.falsy(Path.exists(test_dir_file_path))
        assert.are.same("123", Path.read(test_file_path))
    end)
    it("dir", function()
        local start_dir = Path.joinpath(test_dir_path, "start")
        local start_f = Path.joinpath(start_dir, "file.txt")

        local end_dir = Path.joinpath(test_dir_path, "end")
        local end_f = Path.joinpath(end_dir, "file.txt")

        Path.mkdir(start_dir)
        Path.write(start_f, "123")

        Path.rename(start_dir, end_dir)
        assert.falsy(Path.exists(start_dir))
        assert.are.same("123", Path.read(end_f))
    end)
end)

describe("is_file_like", function()
    it("+: extention", function()
        assert(Path.is_file_like("a.md"))
    end)
    it("-: no extention", function()
        assert.falsy(Path.is_file_like("a"))
    end)
end)

describe("is_dir_like", function()
    it("+: no extention", function()
        assert(Path.is_dir_like("a"))
    end)
    it("-: extention", function()
        assert.falsy(Path.is_dir_like("a.md"))
    end)
end)
